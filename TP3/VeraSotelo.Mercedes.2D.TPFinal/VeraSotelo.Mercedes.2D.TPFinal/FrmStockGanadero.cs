...
namespace VeraSotelo.Mercedes._2D.TPFinal
{
    public partial class FrmStockGanadero : Form
    {
        ...
        public FrmStockGanadero(string rutaStock)
        {
            this.rutaStock = rutaStock;
            InitializeComponent();
        }
        ...
        /// <summary>
        /// Sale del formulario guardando los cambios en el archivo Json de stock. Antes informa tanto si la operacion ha sido exitosa como si ha habido algun error al guardar los datos.
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnVolver_Click(object sender, EventArgs e)
        {
            try
            {
                Archivos<List<Bovino>>.ExportarDatosAJson(rutaStock, Establecimiento.StockGanadero);
                MessageBox.Show("Los cambios se han guardado exitosamente", "Confirmación", MessageBoxButtons.OK, MessageBoxIcon.Information);
                this.Close();
            }
            catch (Exception ex)
            {
        ...
        {
            dataStock.DataSource = null;
            dataStock.DataSource = Establecimiento.StockGanadero;
            if (dataStock.Rows.Count > 0)
            {
                dataStock.Rows[0].Selected = false;
            }
        }
